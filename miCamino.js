...
for (let i = 0; i < miCamino.length; i++) {
  const element = miCamino.material[i];

  if (i === 0) {
    console.log(`lo primero que debo llevar son ${element.nombre} y es ${element.descripcion}`);
  }
  else if (i === 1) {
    console.log(`lo segundo que debo llevar son ${element.nombre} y es ${element.descripcion}`);
  }
  else if (i == 2) {
    console.log(`lo tercero que debo llevar son ${element.nombre} y es ${element.descripcion}`);
  }
  else {
...
let subiendo = miCamino.subiendoAlEverest[0][0].campoBase[0].campoI.campoII.campoIII.campoIV
const mensajeFinal = []
for (let i = 0; i < subiendo.length; i++) {
  const element = subiendo[i];
